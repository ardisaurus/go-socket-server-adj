...
func socketIoHandle(io *socketio.Io) {
	io.OnConnection(func(socket *socketio.Socket) {
		...
		io.To("qr-room").Emit("qr-item", socket.Id+" joined the room", "server message")

		// Listens for "test" event and responds with the same data (Echo)
		socket.On("qr-room-broadcast", func(event *socketio.EventPayload) {
			socket.To("qr-room").Emit("qr-item", event.Data...)
		})
...
func usingWithGin() {
	...
	router.Use(GinMiddleware("http://localhost:5173, https://teams.dashboard-dev.prospace.io:3000/, https://teams.dashboard-dev.prospace.io/, https://teams.dashboard-staging.prospace.io/, https://teams.dashboard.prospace.io/"))
